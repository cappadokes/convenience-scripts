...
do
    for input in /workspace/benchmarks/minimalloc/*.csv; do
        ...
        start_time=$(date +%s%N)
        timeout 3m ./minimalloc/minimalloc --capacity=1048576 --input=$input --output=/workspace/results/time-makespan/minimalloc-benchmarks/minimalloc/csv-out/$new_filename
        ret=$?
        end_time=$(date +%s%N)
