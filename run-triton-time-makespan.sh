...
do  
    ...
    for input in /workspace/benchmarks/minimalloc/*.csv; do
        ...
        BASE_PATH=/workspace/results/time-makespan/minimalloc-benchmarks/triton TRACE_NAME=$filename_no_ext /workspace/triton/triton_packer $input
    done
    echo "" >> $time_file
done
...
do  
    for input in /workspace/benchmarks/mindspore/*.csv; do
        ...
        BASE_PATH=/workspace/results/time-makespan/mindspore-benchmarks/triton TRACE_NAME=$filename_no_ext /workspace/triton/triton_packer $input
    done
    echo "" >> $time_file
done
